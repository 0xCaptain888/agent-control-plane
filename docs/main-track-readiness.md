...
| --- | --- | --- | --- |
| Rebalancing | RebalanceGuard Agent | ERC-8004 identity `1902`; domain task activity still required | 10% max drift, 15% max turnover, approval before execution |
| Grid Trading | SafeSwap Agent | Live BNB Testnet identity and activity proof | 50 USDT max, 50 bps slippage, BNB/USDT only |
| Yield Optimisation | YieldScout Agent | ERC-8004 identity `1903`; domain task activity still required | APY delta required, exposure cap, approval before rebalance |
| Health Factor Monitoring | HealthGuard Agent | ERC-8004 identity `1904`; domain task activity still required | Alert below 1.35, repay cap, no unsanctioned collateral movement |
...
- Record at least one domain-specific public task/activity proof for each new
  profile; registration alone is not presented as a completed Agent workflow.
