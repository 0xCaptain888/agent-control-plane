...
const tokenAddress = process.env.ARBITRUM_TEST_TOKEN_ADDRESS?.trim();
if (!tokenAddress) {
  console.log(JSON.stringify({ status: "not_configured", message: "Set ARBITRUM_TEST_TOKEN_ADDRESS to an Arbitrum Sepolia ERC-20 or USDC address." }, null, 2));
  process.exit(0);
}
...
const token = new Contract(tokenAddress, ["function approve(address spender,uint256 amount) returns (bool)"], wallet);
...
const createTx = await escrow.createTokenTask(tokenAddress, wallet.address, amount, policyHash, deadline);
