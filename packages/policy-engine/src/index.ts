...
export function evaluatePolicy(action: AgentAction, policy: Policy, context: PolicyContext = {}): PolicyDecision {
  ...
  if (budget && policy.requireApprovalAbove?.[budget.currency] !== undefined && Number(budget.amount) > Number(policy.requireApprovalAbove[budget.currency]) && !context.approvalGranted) {
