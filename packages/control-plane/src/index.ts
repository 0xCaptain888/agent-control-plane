...
import { evaluatePolicy, type Policy, type PolicyDecision } from "../../policy-engine/src/index.js";
...
export class AgentControlPlane {
  ...
  async execute(input: ControlPlaneInput): Promise<ControlPlaneResult> {
    const { action, riskContext = {} } = input;
    const policy = evaluatePolicy(action, this.deps.policy);
  ...
  private receipt(
    ...
    return {
      ...
      execution: execution ? { adapter: execution.adapter, externalId: execution.externalId } : undefined,
